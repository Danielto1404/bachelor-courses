...
happyExpList = Happy_Data_Array.listArray (0,33) ([8576,4288,768,512,0,0,134,67,0,24582,12296,4,4096,34304,0,0,2048,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Expression","Or_Expression","And_Expression","Not_Expression","VAR","OPENED_BRACE","CLOSED_BRACE","IMPLICATION","OR","AND","NOT","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
action_17 (13) = happyShift action_14
...
happyReduction_7 (HappyTerminal (TOKEN_VARIABLE happy_var_1))
	 =  HappyAbsSyn7
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TOKEN_VARIABLE happy_dollar_dollar -> cont 8;
	TOKEN_OPENED_BRACE -> cont 9;
	TOKEN_CLOSED_BRACE -> cont 10;
	TOKEN_IMPLICATION -> cont 11;
	TOKEN_OR -> cont 12;
	TOKEN_AND -> cont 13;
	TOKEN_NOT -> cont 14;
	_ -> happyError' ((tk:tks), [])
