...
tests.base(BY, BS, C, DY, DS, C) :- course(BY, BS, C), course(DY, DS, C), next(BY, BS, DY, DS).
tests.base(1, spring, 'МатАн', 2, fall, 'ДифУр').
tests.base(2, fall, 'МатАн', 2, spring, 'ТФКП').
tests.base(1, spring, 'Прог', 2, fall, 'Web').
tests.base(1, fall, 'АрхЭВМ', 1, spring, 'C++').
/*
?- tests.base(1, fall, 'МатАн', Y, S, C).
   Y / 1  S / spring  C / МатАн
?- tests.base(1, spring, 'МатАн', Y, S, C).
   Y / 2  S / fall  C / МатАн
   Y / 2  S / fall  C / ДифУр
*/

% Рекурсивный поиск достижимых вершин
base_all(BY, BS, BC, DY, DS, DC) :- tests.base(BY, BS, BC, DY, DS, DC).
base_all(BY, BS, BC, DY, DS, DC) :- tests.base(BY, BS, BC, IY, IS, IC), base_all(IY, IS, IC, DY, DS, DC).
/*
