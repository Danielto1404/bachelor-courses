...
В первой строке записано натуральное число 𝑁 N — количество узлов в системе (2≤𝑁≤100).
...
На первой строке входного файла содержится 𝑛 n (2≤𝑛≤100) — число вершин в графе и 𝑚 (0≤𝑚≤400) — количество ребер.
...
Символы обозначают: «#» — гора, «.
» — место, пригодное для постройки стены, «-» — место, не пригодное для постройки стены, «A» и «B» — города A и B.
...
Каждая строка должна начинаться с C i — числа кораблей, которые отправляются из одной системы в другую в этот день.
Далее должны следовать C i пар целых чисел, пара A, B означает, что корабль номер A перемещается из текущей звездной системы в звездную систему B.
...
За игру команды получают очки по следующим правилам:  Если победителя удалось выявить в основное время матча, то ему достаётся 3 очка, а проигравшему — 0.
...
Символ Aij в строке i (1  ≤  i  ≤  N) на позиции j (1  ≤  j  ≤  N) означает результат игры команды номер i с командой номер j и может быть одним из:  'W' — означает, что команда i обыграет команду j в основное время матча; 'w' — команда i обыграет команду j в овертайме; 'l' — команда i проиграет команде j в овертайме; 'L' — команда i проиграет команде j в основное время матча; '.' — если результат игры между командами i и j ещё не определён; '#' — если i равно j, означает отсутствие данного матча, т.к. команда не может играть сама с собой.
