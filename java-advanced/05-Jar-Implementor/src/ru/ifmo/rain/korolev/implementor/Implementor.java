...
public class Implementor implements JarImpler {
    ...
    private static String getArgsAndExceptions(Executable executable) {
        ...
        return "(" +
                joinWithComa(executable.getParameters(), it -> it.toString().replace("$", ".")) +
                ")" +
                (exceptions.length == 0 ? "" : " throws " + joinTypes(exceptions));
    }
    ...
    private static String getExecutable(Executable executable) {
        ...
        stringBuilder.append(TAB).append(Modifier.toString(modifiers)).append(SPACE)
