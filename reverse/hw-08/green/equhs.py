...
var_names = ["v" + str(i) for i in range(0, 20)]

vs = [BitVec(name, 8) for name in var_names]
...
output = [61, 228, 94, 109, 218, 126, 203, 95, 215, 205, 89, 141, 101, 11, 88, 148, 223, 222, 13, 189]

s = Solver()
for i in range(0, 20):
    result = inputs[i][0] * vs[0]
    for j in range(1, 20):
        result += inputs[i][j] * vs[j] % 256
    s.add(result == output[i])
...
print(*list(map(chr, v)))

flag = b'spbctf{glaf_haskell}'
